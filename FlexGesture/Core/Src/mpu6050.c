...
HAL_StatusTypeDef MPU6050_Init(I2C_HandleTypeDef *hi2c)
{
  ...
  status = HAL_I2C_IsDeviceReady(hi2c, MPU6050_I2C_ADDR, 3U, MPU6050_I2C_TIMEOUT_MS);
  if (status != HAL_OK)
  {
    return status;
  }

  status = MPU6050_ReadReg(hi2c, MPU6050_REG_WHO_AM_I, &who_am_i);
  if (status != HAL_OK)
  {
    return status;
  }
...
HAL_StatusTypeDef MPU6050_ReadRaw(I2C_HandleTypeDef *hi2c, MPU6050_RawData_t *data)
{
  ...
  data->ax = MPU6050_MakeInt16(buffer[0], buffer[1]);
  data->ay = MPU6050_MakeInt16(buffer[2], buffer[3]);
  data->az = MPU6050_MakeInt16(buffer[4], buffer[5]);
  data->gx = MPU6050_MakeInt16(buffer[8], buffer[9]);
  data->gy = MPU6050_MakeInt16(buffer[10], buffer[11]);
  data->gz = MPU6050_MakeInt16(buffer[12], buffer[13]);
